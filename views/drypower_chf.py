...
def run():
    ...
    chf_info = pd.read_excel(r'C:\Users\hector\Documents\trading\cftc data\years\dea_fut_chf.xlsx')
    chf_info['NonComm_Positions_Short_All'] = chf_info['NonComm_Positions_Short_All'].map(lambda x: x*(-1))
    df_filtered = chf_info[[
    ...
    fig.update_layout(
        title="Dry Power Indicator (6S=F)",
    ...
    # Description
    st.title("Dry Power Indicator for Swiss Franc Futures (6S=F)")
    st.subheader("From 2015 to 2025")
    st.markdown(
        """
        This chart shows a classic DP indicator for Swiss Franc futures. The individual long and 
