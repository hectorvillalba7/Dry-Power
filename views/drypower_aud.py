...
def run():
    ...
    aud_info = pd.read_excel(r'C:\Users\hector\Documents\trading\cftc data\years\dea_fut_aud.xlsx')
    aud_info['NonComm_Positions_Short_All'] = aud_info['NonComm_Positions_Short_All'].map(lambda x: x*(-1))
    df_filtered = aud_info[[
    ...
    fig.update_layout(
        title="Dry Power Indicator (6A=F)",
    ...
    # Description
    st.title("Dry Power Indicator for Australian dollar Futures (6A=F)")
    st.subheader("From 2015 to 2025")
    st.markdown(
        """
        This chart shows a classic DP indicator for Australian dollar futures. The individual long and 
