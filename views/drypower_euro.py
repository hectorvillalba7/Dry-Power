...
def run():
    ...
    euro_info = pd.read_excel(r'C:\Users\hector\Documents\trading\cftc data\years\dea_fut_euro.xlsx')
    euro_info['NonComm_Positions_Short_All'] = euro_info['NonComm_Positions_Short_All'].map(lambda x: x*(-1))
    df_filtered = euro_info[[
    ...
    fig.update_layout(
        title="Dry Power Indicator (6E=F)",
    ...
    # Description
    st.title("Dry Power Indicator for Euro Futures (6E=F)")
